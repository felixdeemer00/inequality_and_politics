...
# Imported RDS files to speed up the app, so it wouldn't have to generate new
# plots each time. In addition, each plot needed to be formatted slightly 
# differently to be ideal, so saving the plots generated by the two R scripts
...
# differently-named set of outputs. This was so that two posterior distributions
# could be generated and displayed at once, as the model is not as informative
# if it cannot be compared to a different set of inputs.

# For each slider or dropdown list, I selected the range of possible values 
# based on the inputs to the model that would be used to generate the posterior
# distribution. The default values were set to ones that would show a moderate
# level of inequality, which could in turn be varied significantly in either 
# direction.
...
# Nested ifelse() statements were used so that the app could display one of a 
# much wider number of plots than would be possible with a single ifelse().
# This particular code chunk is for the 'Observations' tab, displaying 
# different plots based on the variable the user wishes to explore.
...
# This section of code is used to produce the output for the 'Model in Action'
# tab. Given that we know the basic parameters of the graph (the range of 
# values that could be displayed at any one moment is within a given range,
# as set by the scale_x_continuous() function), we can use a single ggplot()
# function without needing to save the graphs as RDS files. 
        
# I used the as.factor() function in order to convert the text / numerical 
# inputs from the sidebar inputs into the factor form required by the model.
